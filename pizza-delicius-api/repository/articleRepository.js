import {ArticleModel} from '../models/articleModel.js';
import {prepareSortObject} from "../utils/prepareSortObject.js";

async function create(data) {
  return await new ArticleModel(data).save()
}

async function list(sortField, onlyEnabled = true) {
  const preparedSortField = prepareSortObject(sortField)
  const params = {}
  if (onlyEnabled) params.published = true
  return await ArticleModel.find(params).sort(preparedSortField)
}

async function getItem(id, onlyEnabled = true) {
  const params = { _id: id }
  if (onlyEnabled) params.published = true
  return await ArticleModel.findOne(params).exec()
}

async function getItemBySlug(slug, onlyEnabled = true) {
  const params = { slug }
  if (onlyEnabled) params.published = true
  return await ArticleModel.findOne(params).exec()
}

async function deleteItem(id) {
  return await ArticleModel.findOneAndDelete({ _id: id }).exec()
}

async function updateItem(id, data) {
  return await ArticleModel.findOneAndUpdate({ _id: id }, data, { new: true }).exec()
}

export const articleRepository = {
  list,
  create,
  getItem,
  getItemBySlug,
  deleteItem,
  updateItem
}
